...
//		ConfigurableApplicationContext 인터페이스를 이용해 Environment 객체 생성
		ConfigurableEnvironment env = ctx.getEnvironment();
//		getPropertySources() 를 이용해 프로퍼티 소스 객체 생성
		MutablePropertySources propertySources = env.getPropertySources();
		try {
//			ResourcePropertySource 객체를 이용해서 외부파일 정보 가져와 프로퍼티 소스에 추가
...
//		객체 생성전과 생성시 해당되는 메소드 호출 -> 프로퍼티 값들 저장
